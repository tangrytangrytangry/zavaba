...
module.exports = {
	...
	eventList: function (req, res, cbEvents) {
		...
		*/
		...
		var queryDates = Activity.find({ date: { $gte: date6MonthsAgoNumber8 } }, 'date item active');
	...
	oneEvent: function (req, res, cbOneEvent) {
		...
		// Using the promise returned from executing a query
		var queryOneEvent = Activity.find({ date: { $eq: eventDate }, item: { $eq: eventNumber } }, 'date item active data');
