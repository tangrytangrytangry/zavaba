...
// Filter events if search value is filled in
function filterEvents(parObjEventList, parFromDateNumber8, parFilterObject, cbEventList) {

	let doFilterEvents = false, filterValue = "", isEventProper = false, removedEvent = {};
	...
	if (parFilterObject) {
		if (parFilterObject.searchtext) {
			if (parFilterObject.searchtext != "") {
				doFilterEvents = true;
				filterValue = parFilterObject.searchtext.trim().toLocaleUpperCase();
			}
		}
	}
	...
	if (doFilterEvents) {
		...
		promiseDesc.then(function (descList) {
			...
			for (let idx = 0; idx < parObjEventList.length; idx++) {
				...
				// If search date - it is proper event
				if (filterValue.indexOf(eventDate.toString()) >= 0) {
					continue;
				}
				...
				for (let index = 0; index < descList.length; index++) {
					...
					if (eventDate == descDate && eventNumber == descNumber) {
						...
						descText = descList[index].data.text.trim().toLocaleUpperCase();

						if (descText.indexOf(filterValue) >= 0) {
							isEventProper = true;
							break;
						} // if (descText.indexOf(filterValue < 0))
				...
				// Remove unproper event 
				if (!isEventProper) {
					removedEvent = parObjEventList.splice(idx, 1);
					idx = idx - 1;
				} // if (!isEventProper)
